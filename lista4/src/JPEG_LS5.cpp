...
void JPEG_LS5::calculateEntropy(std::vector<std::vector<Pixel>> &image) {
    ...
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            ...
            uint8_t red = currPixel.red - (north.red + (west.red - northwest.red) / 2);
            uint8_t green = currPixel.green - (north.green + (west.green - northwest.green) / 2);
            uint8_t blue = currPixel.blue - (north.blue + (west.blue - northwest.blue) / 2);
