...
void JPEG_LS6::calculateEntropy(std::vector<std::vector<Pixel>> &image) {
    ...
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            ...
            uint8_t red = currPixel.red - (west.red + (north.red - northwest.red) / 2);
            uint8_t green = currPixel.green - (west.green + (north.green - northwest.green) / 2);
            uint8_t blue = currPixel.blue - (west.blue + (north.blue - northwest.blue) / 2);
